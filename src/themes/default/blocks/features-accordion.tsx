...
export function FeaturesAccordion({
  ...
  const hasImages = section.items?.some((item) => item.image?.src);
      ...
      <div className="container space-y-8 overflow-x-hidden px-2 sm:px-6 md:space-y-16 lg:space-y-20 dark:[--color-border:color-mix(in_oklab,var(--color-white)_10%,transparent)]">
        ...
        <div
          className={cn(
            ...
            hasImages ? 'md:grid-cols-2' : 'mx-auto max-w-3xl'
          )}
        >
            ...
            >
              {section.items?.map((item, idx) => (
                ...
                >
                  ...
                  <AccordionContent className="text-muted-foreground pr-8 leading-6">
                    {item.description}
                  </AccordionContent>
                </AccordionItem>
              ))}
          ...
          {hasImages && (
            <ScrollAnimation delay={0.2} direction="right">
              ...
              <div className="bg-card border-border relative flex min-w-0 flex-shrink overflow-hidden rounded-2xl border p-3">
                ...
                <div className="bg-secondary relative aspect-76/59 w-full min-w-0 rounded-xl sm:w-[calc(3/4*100%+3rem)]">
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      className="border-border size-full overflow-hidden rounded-xl border shadow-none"
                    >
                      <LazyImage
                ...
                <BorderBeam
                  duration={6}
                  size={200}
                  className="via-primary/50 from-transparent to-transparent"
                />
              </div>
            </ScrollAnimation>
          )}
