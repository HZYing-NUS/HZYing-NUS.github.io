...
export async function getSettingGroups() {
  ...
  const settingGroups: SettingGroup[] = [
    ...
    {
      name: 'webtools_ai',
      title: t('groups.webtools_ai'),
      description:
        'Configure WebTools AI pricing and optional web search. Secret API keys stay in environment variables.',
      tab: 'ai',
    },
    {
      name: 'replicate',
...
export async function getSettings() {
  ...
  "premium-yearly": "prod_xxx"
    ...
    {
      name: 'ai_web_search_credit_cost_usd',
      title: 'Web search provider Credit cost (USD)',
      type: 'number',
      placeholder: '0.008',
      tip: 'Tavily PAYG cost for one provider Credit. Basic search uses 1 provider Credit and advanced search uses 2.',
      group: 'webtools_ai',
      tab: 'ai',
      attributes: { min: '0', step: '0.000001' },
    },
    {
      name: 'ai_web_search_depth',
      title: 'Web search depth',
      type: 'select',
      value: 'basic',
      options: [
        { title: 'Basic · 1 Tavily Credit', value: 'basic' },
        { title: 'Advanced · 2 Tavily Credits', value: 'advanced' },
      ],
      tip: 'The provider returns actual usage. That cost is added to the model cost before applying the 65% gross-margin multiplier.',
      group: 'webtools_ai',
      tab: 'ai',
    },
    {
      name: 'ai_web_search_api_key_env',
      title: 'Web search API key environment variable',
      type: 'text',
      value: 'TAVILY_API_KEY',
      placeholder: 'TAVILY_API_KEY',
      tip: 'Store the real key in .env or Vercel. Save only its environment variable name here.',
      group: 'webtools_ai',
      tab: 'ai',
    },
    {
      name: 'ai_web_search_url',
