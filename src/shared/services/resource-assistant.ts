...
export async function retrieveAssistantSources(question: string, locale: string) {
  ...
  const terms = Array.from(
    new Set(question.toLowerCase().match(/[a-z0-9]+|[\u4e00-\u9fff]{2,4}/g) || [])
  );
  const score = (text: string) => {
    const haystack = text.toLowerCase();
    return terms.reduce(
      (total, term) => total + (haystack.includes(term) ? Math.min(term.length, 4) : 0),
      0
    );
  };
  ...
  const sources: AssistantSource[] = [
    ...rankedResources.filter(({ score }) => score >= minimumScore).slice(0, 8).map(({ item }) => ({ type: 'resource' as const, title: item.name, url: `/${locale}/resources/${item.slug}`, excerpt: `${item.summary}\n${item.reason}\n${item.useCase}` })),
    ...rankedCollections.filter(({ score }) => score >= minimumScore).slice(0, 5).map(({ item }) => ({ type: 'collection' as const, title: item.title, url: `/${locale}/collections/${item.slug}`, excerpt: `${item.summary}\n${item.content}` })),
    ...rankedArticles.filter(({ score }) => score >= minimumScore).slice(0, 5).map(({ item }) => ({ type: 'article' as const, title: item.title || item.slug, url: `/${locale}/blog/${item.slug}`, excerpt: item.summary })),
  ];
