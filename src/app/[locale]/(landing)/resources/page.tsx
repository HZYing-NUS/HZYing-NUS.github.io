...
export default async function ResourcesPage({
  ...
  const visibleResources = tag
    ? resources.filter((resource) =>
        resource.tags.some((item) => item.id === tag)
      )
    : resources;
  const resourceTypes = Array.from(
    new Set(resources.map((resource) => resource.resourceType).filter(Boolean))
  );
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-16 md:py-24">
      ...
      <section className="bg-muted/30 mt-10 rounded-3xl border p-5">
        <FilterLinks
          label={isZh ? '类型' : 'Type'}
          items={resourceTypes.map((item) => ({ id: item, name: item }))}
          value={type}
          allLabel={isZh ? '全部资源' : 'All resources'}
          allHref={queryFor({ type: '' })}
          hrefFor={(item) => queryFor({ type: item.id })}
        />
        <FilterLinks
          label={isZh ? '阶段' : 'Stage'}
          items={filters.stages}
          value={stage}
          allLabel={isZh ? '全部阶段' : 'All stages'}
          allHref={queryFor({ stage: '' })}
          hrefFor={(item) => queryFor({ stage: item.id })}
        />
        <FilterLinks
        ...
        <FilterLinks
          ...
          items={filters.tags}
          value={tag}
          allLabel={isZh ? '全部标签' : 'All tags'}
          allHref={queryFor({ tag: '' })}
          hrefFor={(item) => queryFor({ tag: item.id })}
        />
        <FilterLinks
          label={isZh ? '价格' : 'Pricing'}
          items={priceTypes.map((item) => ({ id: item, name: item }))}
          value={pricing}
      ...
      {visibleResources.length ? (
        <section className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {visibleResources.map((resource) => (
            <article
              key={resource.slug}
              className="bg-background flex flex-col rounded-3xl border p-6 shadow-sm"
            >
              ...
              <h2 className="mt-4 text-xl leading-snug font-semibold">
                {resource.name}
              </h2>
              ...
              <Link
                ...
                className="text-primary mt-6 inline-flex text-sm font-medium"
              >
                {isZh ? '查看资源' : 'View resource'}
              </Link>
            </article>
          ))}
        </section>
...
function FilterLinks({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 py-2">
      ...
      <Link
        href={allHref}
        className={`rounded-full px-3 py-1.5 text-xs font-medium ${!value ? 'bg-primary text-primary-foreground' : 'bg-background border'}`}
      >
        {allLabel}
      </Link>
      {items.map((item) => (
        <Link
          key={item.id}
          href={hrefFor(item)}
          className={`rounded-full px-3 py-1.5 text-xs font-medium ${value === item.id ? 'bg-primary text-primary-foreground' : 'bg-background border'}`}
        >
          {item.name}
        </Link>
      ))}
