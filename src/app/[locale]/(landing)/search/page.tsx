...
export default async function SearchPage({
  ...
  const profileMatches = Boolean(profile && (!keyword || JSON.stringify(profile).toLowerCase().includes(keyword)));
  ...
  const resultCount = resources.length + collections.length + posts.length + Number(profileMatches);
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-16 md:py-24">
      ...
      <section className="mt-8 space-y-10">
        ...
        {profileMatches ? <div><h2 className="text-2xl font-semibold">{isZh ? '关于我' : 'About'}</h2><article className="mt-4 rounded-3xl border p-6"><h3 className="text-xl font-semibold">{isZh ? '梓颖的个人介绍与经历' : 'Ziying profile and experience'}</h3><p className="text-muted-foreground mt-3 text-sm leading-6">{isZh ? '教育、项目、论文、奖项、联系方式与证明材料。' : 'Education, projects, publications, awards, contact details, and supporting materials.'}</p><Link href={`/${locale}/about`} className="text-primary mt-4 inline-flex text-sm font-medium">{isZh ? '查看关于我' : 'View profile'}</Link></article></div> : null}
